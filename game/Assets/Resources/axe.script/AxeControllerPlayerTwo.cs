...
public class AxeControllerPlayerTwo : MonoBehaviour {
	...
	private characterTwoController characterContr;
	...
	void GetPlayerControllers(){
		...
		characterContr = player.GetComponent<characterTwoController>();
	...
	private void CheckCollision() {
		...
		if(hitDown.collider != null){//there is something under me
			...
			if(!axeHasLanded && hitDown.collider.tag != "PlayerTwo" && hitDown.collider.tag != "PlayerOne"){
		...
		if(hitUp.collider != null){//there is something above me
			...
			if(!axeHasLanded && hitUp.collider.tag != "PlayerTwo" && hitUp.collider.tag != "PlayerOne"){
		...
		if(hitLeft.transform != null){//there is something on my left side
			...
			if(!axeHasLanded && hitLeft.collider.tag != "PlayerTwo" && hitLeft.collider.tag != "PlayerOne"){
		...
		if(hitRight.collider != null){//there is something on my right side
			...
			if(!axeHasLanded && hitRight.collider.tag != "PlayerTwo" && hitRight.collider.tag != "PlayerOne"){
	...
	void StandardHitChecks(RaycastHit2D hit){
		...
		if(!axeHasLanded && hit.collider.gameObject.tag == "PlayerOne"){
	...
	void StartReviveTimer(){
		...
		playerRevive.APlayerDied(1);
	}

	void DeactivatePlayerOneAttack(){

		axeTrowing.APlayerDied(1);
	}
